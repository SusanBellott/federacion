...
const cursosFiltrados = computed(() => {
  return props.cursos.data?.filter(curso => curso.fecha_inicio_inscripcion <= hoy);
});
...
<template>
    ...
    <div v-if="mostrarAlertaEdicion">
        <AlertNotification title="¡Registro editado correctamente!" action="update" />
    </div>
    <div v-if="mostrarAlertaEliminacion">
        <AlertNotification title="¡Registro eliminado correctamente!" action="delete" />
    </div>
</div>
                ...
                <div class="flex-auto px-0 pt-0 pb-2">
                    <div class="p-0 overflow-x-auto">
                        <table class="items-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                            ...
                            <tbody>
                                <tr v-for="(micurso, index) in miscursos.data" :key="micurso.id_inscripcion" class="border-b dark:border-white/40">
                                  <td class="w-[100px] p-2 text-center align-middle bg-transparent border-b dark:border-white/40 text-[11px] font-semibold text-gray-700 dark:text-white dark:opacity-80 whitespace-normal break-words uppercase">
                                    ...
                                    <td class="w-[100px] p-2 text-center align-middle bg-transparent border-b dark:border-white/40 text-[11px] font-semibold text-gray-700 dark:text-white dark:opacity-80 whitespace-normal break-words uppercase">
                                        <p class="text-xs font-semibold leading-tight text-gray-700 dark:text-white dark:opacity-80 max-w-xs break-words">
                                            {{ micurso.curso ? micurso.curso.nombre : 'N/A' }}
                                        </p>
                                    </td>
                                    <td class="w-[100px] p-2 text-center align-middle bg-transparent border-b dark:border-white/40 text-[11px] font-semibold text-gray-700 dark:text-white dark:opacity-80 whitespace-normal break-words uppercase">
                                        <p class="text-xs font-semibold leading-tight text-gray-700 dark:text-white dark:opacity-80 max-w-xs break-words">
                    ...
                    <div class="px-4 py-3 border-t border-gray-200 dark:border-white/10">
                        ...
                        <Pagination :pagination="miscursos" :filters="filters" />
